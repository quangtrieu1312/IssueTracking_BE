...
import com.trieutruong.project.job.AlertTicketJob;
import com.trieutruong.project.joblistener.AlertTicketJobListener;
import com.trieutruong.project.repository.TicketRepository;
...
public class QuartzSchedulerServiceImpl implements QuartzSchedulerService {
	...
	private Trigger createTrigger(Ticket ticket) throws IOException {
		...
		return TriggerBuilder.newTrigger().withIdentity(ticket.getTicketId(), ticket.getOwnerId())
				.withSchedule(CronScheduleBuilder.cronSchedule(ticket.getAlert().getCronExpression())
						.inTimeZone(TimeZone.getTimeZone(ticket.getAlert().getTimezone())))
				.forJob(jobKey).build();
	}
